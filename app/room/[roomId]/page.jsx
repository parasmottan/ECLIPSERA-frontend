...
// A simple Popup component for warnings and confirmation
const Popup = ({ visible, type, message, onConfirm, onCancel }) => {
...
      <div className="bg-[#1C1C1C] p-6 rounded-lg shadow-2xl max-w-sm w-full border border-white/10">
        <h3 className="text-xl font-bold mb-4">
          {isConfirm ? "Confirm Action" : "Warning"}
        </h3>
        <p className="text-white/80 mb-6">{message}</p>
        <div className="flex justify-end gap-3">
          {isConfirm && (
            <button
              onClick={onCancel || (() => onConfirm())} // Use onConfirm as cancel if onCancel is missing
...
          <button
            onClick={onConfirm}
            className={`px-4 py-2 rounded-full font-semibold ${isConfirm ? "bg-red-600" : "bg-blue-600"}`}
          >
...
    message: "",
    onConfirm: null,
    onCancel: null, // Added onCancel for confirmation popups
  });
...
      if (res.status === 404) {
        // 🔥 FIXED: Agar 404 aaya toh setValid(false) karo, redirect sirf UI mein hoga.
        setValid(false);
        // router.push("/"); // ❌ Redirect UI mein handle hoga
        return;
      }
...
      // NOTE: process.env variables should be correctly set in your environment
...
        }
      },
      onCancel: () => setPopup({ visible: false }), // Added cancel handler
    });
  };
...
  // ✅ FIXED: Agar room ID invalid hai, toh redirect karo
  if (valid === false) {
    router.push("/");
    return null; // Component render na ho isliye null return
  }
  
...
      <Popup {...popup} onCancel={() => setPopup({ visible: false })} />
    </div>
  );
}